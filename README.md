...
- 已接入品牌蘑菇图标，首页、编辑器顶部和项目信息弹窗共用统一品牌入口
- 补齐基础 SEO 元信息、结构化数据、`robots.txt` 与 `sitemap.xml`
- 图片与生成模块重排，上传图片与实时预览入口分离，减少左栏首屏占位
- 高级颜色区收敛为更紧凑的默认两行显示，并补充颜色编号与中文名称
- 近期若干右栏和图片区样式调整已收口到当前版本，避免继续保留半成品布局
- 已接入三套可切换界面主题，并补齐主题持久化、顶部切换入口和中间画布 / 图片预览的主题同步
- 中间画布升级为高 DPI 渲染，放大后的网格、边线和格子清晰度明显改善
- 主题系统已完成第一轮变量收口，后续视觉打磨项已补充到开发计划
...
`v1.0.5` 在 `v1.0.4` 的基础上继续推进主题系统落地，完成三套主题切换、高 DPI 画布渲染、主题变量收口和一轮工作台视觉层级打磨，重点落在“界面已可切换主题且不影响拼豆业务色”以及“中间画布放大后不再明显发糊”。
...
- 更好的颜色筛选与库存导向工作流
- 功能名称：主题系统视觉打磨收口
  实现方式：当前主题系统已支持 `工坊暖白 / 纸面冷白 / 夜间深色` 三套主题切换，但不同主题下仍需要继续做人眼导向的视觉微调。后续按模块逐块收口，优先处理右侧高级颜色区、左侧图片与生成区、中间工具栏与状态栏、各类弹窗与帮助中心。重点不是继续补变量，而是调卡片亮度层级、边框存在感、按钮选中态强度、禁用态对比度、说明文字灰度、深色主题下的主次关系，确保每套主题不只是“能切换”，而是都有完整、稳定、清晰的工作台观感。
- 功能名称：智能背景去除
