...
- 图片预览默认回到原图视图，实时预览改为可选开关，避免左侧预览持续重算带来的卡顿
- 帮助中心入口与内容联动，点击各模块问号后会直接定位到对应说明
- 图纸导出清晰度提升，格子标签继续使用拼豆色号，方便直接照图施工
- 裁切空白逻辑补强，对边缘浅色留白更稳，减少裁不掉或误留白边的情况
- 顶部项目信息、版本状态、反馈入口继续收敛，编辑器入口信息更统一
...
`v1.0.3` 在 `v1.0.2` 的基础上继续修正生成与预览链路，重点收敛图片预览、帮助联动、图纸导出和裁切空白这几条最近频繁调整的工作流。
